...
class SummaryComputationStudent(SummaryComputationInterface):
    ...
    def fetch_registrants(self, registrant_ids, sr_session) -> List[G2PStudentRegistry]:
        return sr_session.query(G2PStudentRegistry).filter(G2PStudentRegistry.id.in_(registrant_ids)).all()
    ...
    def compute_and_persist_summary(self, registrant_ids, base_summary, sr_session: Session, eee_session: Session):
        ...
        students_age = [self.calculate_age(student.date_of_birth) for student in registrants if student.date_of_birth]
